...
PARAM <- list()
PARAM$experimento <- "FE7310_FINAL_04"

PARAM$exp_input <- "DR7210_FINAL_00"
...
PARAM$RandomForest$semilla <- 100999 # cambiar por la propia semilla
...
PARAM$CanaritosAsesinos$semilla <- 101021
...
if (PARAM$lag1) {
  ...
  for (vcol in cols_lagueables) {
    dataset[, paste0(vcol, "_acc2") := get(paste0(vcol, "_delta1")) - shift(get(paste0(vcol, "_delta1")), 1, NA)]
  }
